const { body, param, headers } = require("express-validator");
...
class Validator {
  ...
  validateParam() {
    return [
      param("idUser").isUUID().withMessage("idUser толжен быть в формате UUID"),
    ];
  }

  validateIdUser() {
    return [
      headers.("idUser").isUUID().withMessage("idUser толжен быть в формате UUID"),
    ];
  }
}
