...
class TodoController {
  async getTodos(req, res) {
    let login = jwt.decode(result, process.env.ACCESS_TOKEN_SECRET);
    let idUser = req.users.find()
    // let result = req.todos
    //   .filter((item) => req.body.id === item.idUser)
    //   .sort((a, b) => a.id - b.id);
    // res.status(200).send(result);
    let result = req.headers.authorization.split(` `)[1]
    
    res.send(jwt.decode(result, process.env.ACCESS_TOKEN_SECRET))
  }
